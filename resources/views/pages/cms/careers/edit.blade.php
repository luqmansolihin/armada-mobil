        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Careers | Update</h3>
                </div>

                <form class="form-horizontal" action="{{ route('cms.careers.update', $career->id) }}" method="POST">
                    @method('PUT')
                    @csrf
                    <div class="card-body">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                <textarea name="description" class="form-control @error('description') is-invalid @enderror" id="description"
                                    placeholder="Job Description" rows="4" required>{{ old('description', $career->description) }}</textarea>
                                @error('description')
                                    <span class="error invalid-feedback">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                <textarea name="requirement" class="form-control @error('requirement') is-invalid @enderror" id="requirement"
                                    placeholder="Requirement" rows="4" required>{{ old('requirement', $career->requirement) }}</textarea>
                                @error('requirement')
                                    <span class="error invalid-feedback">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-4">
                                <input type="text" id="registration_from" name="registration_from"
                                    ...
                                    value="{{ old('registration_from', $career->registration_from) }}"
                                    data-target="#registration_from" data-toggle="datetimepicker" autocomplete="off"
                                    required />
                            ...
                            <div class="col-sm-4">
                                <input type="text" id="registration_to" name="registration_to"
                                    ...
                                    value="{{ old('registration_to', $career->registration_to) }}"
                                    data-target="#registration_to" data-toggle="datetimepicker" autocomplete="off"
                                    required />
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-4">
                                <input type="number" id="minimal_age" name="minimal_age"
                                    ...
                                    value="{{ old('minimal_age', $career->minimal_age) }}" required />
                                @error('minimal_age')
                                    <span class="error invalid-feedback">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="col-sm-4">
                                <input type="number" id="maximal_age" name="maximal_age"
                                    ...
                                    value="{{ old('maximal_age', $career->maximal_age) }}" required />
                                @error('maximal_age')
                                    <span class="error invalid-feedback">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="form_field_outer">
                            @foreach (old('careers', $career->careerPlacements) as $index => $careerPlacement)
                                <div class="form_field_outer_row">
                                    <div class="form-group row">
                                        <label for="placement_{{ $loop->index }}"
                                            class="col-sm-2 col-form-label">Placement</label>
                                        <div class="col-sm-6 mb-1">
                                            <select
                                                class="form-control select2 @error('careers.' . $loop->index . '.placement') is-invalid @enderror"
                                                id="placement_{{ $loop->index }}"
                                                name="careers[{{ $loop->index }}][placement]" required>
                                                @foreach ($cities as $city)
                                                    <option value="{{ $city->id }}"
                                                        @if ((old('careers') ? $careerPlacement['placement'] : $careerPlacement->city_id) == $city->id) selected @endif>
                                                        {{ $city->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                            @error('careers.' . $loop->index . '.placement')
                                                <span class="error invalid-feedback">{{ $message }}</span>
                                            @enderror
                                        </div>
